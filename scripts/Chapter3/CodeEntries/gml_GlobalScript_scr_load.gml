function scr_load()
{
    ...
    if (global.chapter == 3)
    {
        if (global.flag[1273] == 1)
        {
            ...
            var krs_word = get_chapter_lang_setting("krs_word", ["KRS"]);
            var alphanumericarray = [];
            for (var i = 0; i < string_length(alphabet); i++)
                alphanumericarray[i] = string_char_at(alphabet, i + 1);
            
            global.flag[1012] = array_get_item_index(alphanumericarray, string_char_at(krs_word[0], 1));
            global.flag[1013] = array_get_item_index(alphanumericarray, string_char_at(krs_word[0], 2));
            global.flag[1014] = array_get_item_index(alphanumericarray, string_char_at(krs_word[0], 3));
        }
        else
        {
