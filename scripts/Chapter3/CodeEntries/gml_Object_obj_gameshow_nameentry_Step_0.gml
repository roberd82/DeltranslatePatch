...
if (con == 0 || con == 1 || con == 2)
{
    ...
    if ((uholdbuff % buffmodrate) == 0)
    {
        ...
        letter[con] = (letter[con] - 1 + maxletter) % maxletter;
        ...
        if (con == 2)
        {
            ...
            while (nextletter) {
                letter[con] = (letter[con] - 1 + maxletter) % maxletter;
                nextletter = scr_nameentrybadcheck();
            }
        }
    }
    else if ((dholdbuff % buffmodrate) == 0)
    {
        ...
        letter[con] = (letter[con] + 1) % maxletter;
...
if (con == 3)
{
    ...
    if (array_includes(get_chapter_lang_setting("bad_word_parent_control", ["PEE", "POO"]), scr_gameshowname()))
    {
    ...
    if (isbadword)
    {
        ...
        else
        {
            ...
            snd_play(snd_error);
            var bad = stringsetloc("BAD", "obj_gameshow_nameentry_slash_Step_0_gml_259_0")
            ...
            letter[0] = array_get_item_index(alphanumericarray, string_char_at(bad, 1));
            letter[1] = array_get_item_index(alphanumericarray, string_char_at(bad, 2));
            letter[2] = array_get_item_index(alphanumericarray, string_char_at(bad, 3));
            global.flag[1012] = letter[0];
            global.flag[1013] = letter[1];
            global.flag[1014] = letter[2];
