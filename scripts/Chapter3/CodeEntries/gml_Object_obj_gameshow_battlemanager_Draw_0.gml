...
if (con == 3)
{
    ...
    if (rcon == 0)
    {
        ...
        var mid = _xx - 30
        var youget_s = string_copy(yougetRrankstring, 1, string_pos("@", yougetRrankstring) - 1)
        var sobaka_pos = string_pos("@", yougetRrankstring)
        var rank_s = string_copy(yougetRrankstring, sobaka_pos + 1, string_length(yougetRrankstring) - sobaka_pos)
        var w = string_width(youget_s + rank_s) + 32
        ...
        if (rtimer > (dingtime * 4))
            draw_text(mid - w / 2, _yy + (4.5 * mspace) + 30, youget_s);
        
        draw_set_halign(fa_center);
        
        if (rtimer > (dingtime * 4))
            draw_text_transformed_color(mid - w / 2 + string_width(youget_s) + 16, _yy + (4 * mspace) + 28, get_letter_rank(lettergrade), 2, 2, 0, lettergradeblend, lettergradeblend, lettergradeblend, lettergradeblend, 1);
        
        draw_set_halign(fa_left);
        
        if (rtimer > (dingtime * 4))
            draw_text(mid - w / 2 + string_width(youget_s) + 16 + 16, _yy + (4.5 * mspace) + 30, rank_s);
