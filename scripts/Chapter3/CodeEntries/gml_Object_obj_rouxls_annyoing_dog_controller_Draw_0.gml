...
if (con == 3 && timer > 60)
{
    ...
    var dogstring = stringsetloc("ANNOYING#DOG", "obj_rouxls_annyoing_dog_controller_slash_Draw_0_gml_1_0");
    ...
    if (global.lang != "ja")
    {
        ...
        var str_scale = string_width(string_hash_to_newline("ANNOYING#DOG")) / string_width(dogstring)
        
        for (var i = 1; i < 6; i++)
            draw_text_transformed(textposx + (i * 2), textposy + (i * 2), dogstring, 3 * str_scale, 3 * str_scale, angle);
        
        draw_set_color(-1);
        draw_text_transformed(textposx, textposy, dogstring, 3 * str_scale, 3 * str_scale, angle);
    ...
    var revline = scr_84_get_sprite("spr_smashreveal3");
    var bon = 0;
