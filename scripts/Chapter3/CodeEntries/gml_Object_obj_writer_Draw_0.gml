if (view_current == 0)
{
    ...
    for (n = 1; n < pos; n += 1)
    {
        ...
        for (var g = 0; g < array_length(additional_sprites); g++) {
            if (n == additional_sprites[g][0]) {
                if (additional_sprites[g][2] == 0)
                {
                    var writerobj = instance_create(wx, wy, obj_funnytext);
                    writerobj.sprite_index = additional_sprites[g][1];

                    additional_sprites[g][2] = 1
                }
                
                wx += sprite_get_width(additional_sprites[g][1]);
            }
        }

        for (var g = 0; g < array_length(additional_sounds); g++) {
            if (n == additional_sounds[g][0]) {
                if (additional_sounds[g][2] == 0) {
                    snd_play(additional_sounds[g][1])
                    additional_sounds[g][2] = 1
                }
            }
        }
        ...
        if (accept == 1)
        {
            ...
            if (!get_lang_setting("monospace_fonts", false))
            {
                ...
                {
                    wx += (string_width(mychar) * textscale);
                    
                    if (n < string_length(mystring))
                    {
                        if (mychar == "?" && string_char_at(mystring, n + 1) == ".")
                        {
                            if (myfont == scr_84_get_font("main"))
                                wx -= 2;
                            
                            if (myfont == scr_84_get_font("mainbig"))
                                wx -= 4;
                        }
                    }
                }
