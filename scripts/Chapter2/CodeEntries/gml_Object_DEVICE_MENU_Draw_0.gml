...
if (MENU_NO >= 0)
{
    for (i = 0; i < 3; i += 1)
    {
        ...
        {
            var NOWNAME = (NAME[i] == "[EMPTY]") ? stringsetloc("[EMPTY]", "scr_84_load_ini_slash_scr_84_load_ini_gml_13_0") : NAME[i];
...
var xx_ch1 = xx_left_edge - string_width(CH1TEXT);
...
if (MENU_NO >= 0)
{
    ...
    if (MENU_NO == 0 || MENU_NO == 1)
    {
        ...
        draw_text_shadow(xx_ch1, 210, CH1TEXT);
        draw_set_color(COL_A);
    ...
    if (TYPE == 1)
    {
        ...
        draw_set_halign(fa_right);
        draw_text_transformed(320, 230, stringsetsubloc("DELTARUNE ~1 (C) Toby Fox 2018-2025 ", version_text, "DEVICE_MENU_slash_Draw_0_gml_202_0"), 0.5, 0.5, 0);
        draw_set_halign(fa_left);
    }
    else
    {
