if (freeplay < 0)
{
    ...
    _ystart = 80;
    draw_text(230, _ystart - _textspacing, stringsetloc("TRACK SELECT", "obj_rhythmgame_slash_Draw_0_gml_1_0"));
    var _difficulty = "";
    ...
    for (i = 0; i < array_length(tracklist); i++)
    {
        if (i == song_id)
        {
            ...
            if (hardmode)
                _difficulty = stringsetloc(" (Hard)", obj_rhythmgame_slash_Draw_0_gml_3_0);
        }
        
        draw_text(210, _ystart + (_textspacing * i), stringsetsubloc("~1 - ~2~3", string_format_zero(i, 2, 0), tracklist[i], _difficulty, "obj_rhythmgame_slash_Draw_0_gml_2_0"));
    ...
    var _confirm = "[Z]";
    var _cancel = "[X]";
    draw_text_transformed(camerax() + 320, cameray() + 240, string_hash_to_newline(stringsetsubloc("~1 Play it!#~2 Nevermind", _confirm, _cancel, "obj_rhythmgame_slash_Draw_0_gml_3_0")), 2, 2, 0);
...
{
    draw_sprite_ext(scr_84_get_sprite("spr_rhythmgame_bg"), bg_con, x, y + 100, image_xscale, image_yscale, image_angle, image_blend, image_alpha);
    draw_set_blend_mode(bm_add);
    draw_sprite_ext(scr_84_get_sprite("spr_rhythmgame_bg"), bg_con, x, y + 100, image_xscale, image_yscale, image_angle, image_blend, 0.1 + (sin(bg_timer / 6) * 0.1 * (kchart_ease * 0.65)));
