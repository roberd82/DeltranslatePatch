    ...
    if (drawmenu)
    {
        ...
        if (m == menu)
        {
            if (optionTopComment[m][menuCoord[m]] != " ")
            {
                ...
                if (display_buttons)
                {
                    var button_text = optionTopComment[m][menuCoord[m]];
                    var button_x1 = xx + 18;
                    var button_x2 = xx + 18;
                    var flag = 1;
                    
                    for (i = 0; i < string_length(button_text); i++)
                    {
                        ...
                        button_x1 += (string_width(string_char_at(button_text, i + 1)) * flag);
                        button_x2 += string_width(string_char_at(button_text, i + 1));
                    }
                    
                    draw_text(xx + 18, yy + 20, string_hash_to_newline(optionTopComment[m][menuCoord[m]]));
                    draw_sprite_ext(scr_getbuttonsprite(global.input_g[4], false), 0, button_x1, yy + 22, 2, 2, 0, c_white, 1);
                    draw_sprite_ext(scr_getbuttonsprite(global.input_g[5], false), 0, button_x2, yy + 22, 2, 2, 0, c_white, 1);
                }
                else
                {
        ...
        for (j = minj; j < maxj; j++)
        {
            ...
            if (optionCommentA[m][j] != " ")
            {
                ...
                if (optionCommentAWidth[m][j] >= 5)
                    textxscale = clamp(optionCommentAWidth[m][j] / string_width(string(optionCommentA[m][j])), 0.5, 1);
            ...
            if (optionCommentB[m][j] != " ")
            {
                ...
                if (optionCommentBWidth[m][j] >= 5)
                    textxscale = clamp(optionCommentBWidth[m][j] / string_width(string(optionCommentB[m][j])), 0.5, 1);
            ...
            if (optionCommentC[m][j] != " ")
            {
                ...
                if (optionCommentCWidth[m][j] >= 5)
                    textxscale = clamp(optionCommentCWidth[m][j] / string_width(string(optionCommentC[m][j])), 0.5, 1);
